...
from .helpers import local_maxima, peak_picking
from .labels import ID_TO_LABEL


def event_frames_to_time(frame_rates, boundary: np.ndarray):
    boundary = np.array(boundary)
    return boundary / frame_rates


def postprocess_functional_structure(logits, config):
    ...
    segments = []
    for (start, _end), label in zip(pred_boundaries, pred_labels):
        segments.append((float(start), str(ID_TO_LABEL[label])))
